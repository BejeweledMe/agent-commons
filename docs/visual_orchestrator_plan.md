...
V1 без отдельного протокольного решения»): протокольное решение принято, риск
принят заказчиком осознанно, и уровень `auto` поставляется **вместе** со всеми
семью ограничителями, а не после них.
...
| Потолок оборота: создания и выводы считаются вместе | там же — цикл create/retire упирается в бюджет |
| Уровень строго убывает при автоматическом создании | там же — третье поколение отказано |
| Каскадный вывод одним действием, отказ целиком при живой работе | там же |
...
| Отложено | Причина |
|---|---|
| **Расширение каталога MCP/tools из UI** | Запись в capability-половину каталога из loopback-surface означает, что bearer-токен расширяет полномочия каждого дочернего процесса. Каталог остаётся операторским файлом; UI показывает его read-only и даёт путь. Это прямой конфликт с Р3, названный явно, а не обойдённый |
| **Индексирование истории (FTS5)** | Механизм без вызывающего не мержится — ADR 0008. Индекс поставляется вместе с полем поиска, которое из него читает, а не раньше. Расчёт объёмов и выбор — в ADR 0009 Q5 |
| **`allowed_action: handoff_work` у связи** | Модель уже несёт типизированное действие, поэтому добавление режима «отдать работу» не потребует менять запись. Сам режим — следующий шаг |
...
| **Ответ на бэклог живой роли из панели** | Существующий communication store — операционный и не входит в снапшот проекции. Ответ на `input_needed` идёт существующим путём; связывание его с панелью — отдельный инкремент со своим продюсером |
