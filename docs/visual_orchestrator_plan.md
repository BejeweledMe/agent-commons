...
| **Запуск делегации из UI (§10, stage-3 MUST-4) и стриминговый шов (MUST-5)** | Панель строит оргструктуру, говорит с ней и отвечает ей, но каждая единица работы всё ещё запускается из CLI (`task create` → `delegation create --on-behalf-of` → `broker run`). Кнопка «запустить роль на задаче X» и потоковый шов — отдельная работа (round-2, оба продуктовых ревьювера) |
