...
| Внутренний DemoRunner (`demo: true` в явно заданном runtime-конфиге): development/test-seam не запускает провайдера; UI его не создаёт и не рекламирует | `tests/runtime/test_demo_mode.py` |
...
счёт. Внутренний `DemoRunner` не является путём панели и не выдумывает вердикт
ревью; он сохраняется только для development и hermetic tests.
