...
### Инкремент E — редизайн панели и библиотека (сделано, 2026-08-13)

Контракт выработан консилиумом (PM + системный SWE + LLM-инженер, синтез с
поправками по коду) после юзабилити-прогона; реализована вся «волна 1».

| Что | Где проверяется |
|---|---|
| Три зоны вместо десяти табов: сайдбар-библиотека (борд, прогоны, Обзор, Скиллы, Инструменты, Каталог агентов, честные заглушки SGR/MCP), борд в центре, чат + очередь внимания справа; «Запись» — вкладка выбранной сущности; найм за «+»; холодный старт с трёхшаговой картой; тултипы на всём; обе локали | `tests/ui/test_stream_and_packaging.py` (CSP/i18n/glyph-гарды), живой прогон обеих локалей |
| Связи на борде: 4 порта у активной роли, резинка, форма (тип, срок-«информативный», причина) → `POST /api/agent-links`; закрытие — записанная смена состояния с ревизией (вкладка «Связи» роли, кнопка в «Записи» узла), слово «удалить» в UI связей отсутствует; ребро-разрешение пунктиром, происхождение — сплошной | `tests/ui/test_readonly_invariant.py` (запечатанная поверхность + one-write-path + HTTP-цикл open/close), живой E2E-прогон |
| `profile_tools` в `GET /api/catalog` — те же наборы, что собирает запуск (бит-в-бит тест против `_worker_tools`); сужение туллов — радио «весь профиль/сузить» по реальному набору, outcome-туллы залочены; зеркальные проверки на найме и reconfigure (несуществующий скилл / чужой тулл → отказ с именами до записи) | `tests/ui/test_catalog_editing.py` |
| Прифлайт конфигурации: роль с исчезнувшим из каталога скиллом видна в очереди внимания до любого запуска | `tests/ui/test_blockers.py` |
| Каталог агентов: шаблоны (`template: true`) как полка с «Нанять из этого»; модалка найма — сначала «новый / из каталога», поля следуют выбору; найм из шаблона шлёт только `from_preset_id`, наследуя профиль/права/бюджет | `tests/ui/test_catalog_editing.py` (наследование через HTTP), живой прогон |
| Demo-режим (`demo: true` в runtime-конфиге): DemoRunner на том же раннер-шве честно закрывает implementation-прогон без провайдера; ревью/верификации не имитируются | `tests/ui/test_launch.py`, `tests/runtime/test_profiles_policy.py` |

### Инкремент F — цепочка приёмки (сделано, 2026-08-13)

Третий слепой прогон вёлся двумя тестерами параллельно, на разных моделях,
в изолированных воркспейсах, так что их находки не пересекались. Оба
независимо упёрлись в одно и то же: прогон доходит до `succeeded`, а принять
работу негде. Разбор
(`docs/audits/2026-08-13-cold-run-usability.md`) показал, что не хватало не
кнопки, а цепочки: `task.accepted` легален только из состояния `review` и
только при одобренном независимом неустаревшем ревью, привязанном к текущей
`effective_revision` задачи и написанном вне принципалов, сделавших работу.

| Что | Где проверяется |
|---|---|
| Три маршрута цепочки: `review-request` доводит задачу до `review` собственными переходами менеджера и открывает независимый запрос; `accept` записывает человеческое решение, а какое ревью годится — решает менеджер; `reopen` возвращает работу. Каждый — тонкий адаптер над тем же `CommonsManager`, каждый запечатан в `MUTATING_ROUTES` вместе со своим тестом | `tests/ui/test_acceptance_chain.py`, `tests/ui/test_readonly_invariant.py` |
| Домен не ослаблен: задача не переходит сама, оператор не может одобрить собственный запрос на ревью, приёмка без одобренного вердикта отказана — и этот отказ закреплён тестом как свойство, а не как пробел | `tests/ui/test_acceptance_chain.py` |
| Отказы жизненного цикла доходят человеческой фразой, называющей следующее действие; каноническая формулировка остаётся в скобках | `tests/ui/test_stream_and_packaging.py` |
| Завершённый прогон с непринятой задачей попадает в очередь внимания одним элементом на задачу | `tests/ui/test_acceptance_chain.py` |
| Прогоны сообщают время старта и длительность из операционного хранилища попыток; часов в replay по-прежнему нет | `tests/ui/test_launch.py` |

Вторая волна того же прогона — юзабилити панели: человеческая сводка вместо
сырого JSON в каждом ящике, глоссарий с одним словом на понятие, глосса рядом
с каждым каноническим значением, ловушка фокуса в модалках, борд без
runtime-узлов по умолчанию, порты связей с 24-пиксельной зоной захвата,
запас +30% на длину строки, тёмные скроллбары и адаптив ниже 900px.
Все — в `tests/ui/test_stream_and_packaging.py`, каждый тест проверен на
падение против файла до изменения.

**Что честно не сделано.** Расхода прогона панель не показывает: потреблённые
`provider_units` в этом коде не записывает никто, поэтому показан только
разрешённый при запуске бюджет, и рядом сказано, что это разрешение, а не
счёт. В demo-режиме цепочка приёмки не закрывается до конца: `DemoRunner`
намеренно не выдумывает вердикт ревью, так что одобренное независимое ревью
может прийти только от настоящего ревьюера — панель говорит это прямо, вместо
того чтобы имитировать приёмку.

### Что отложено, и почему

| Отложено | Причина |
|---|---|
| **Пресеты как отдельная сущность** | Позиция домена не изменилась: пресет — это роль с `template: true`, четвёртый вид записи не заводится. Изменилась подача: инкремент E дал шаблонам полку («Каталог агентов») и режим найма «из каталога» — поверх той же записи роли |
...
| **Истечение связи** | Отменено, а не отложено (2026-08-13). У replay нет часов, а сверяться с ними значило бы проецировать одни и те же события по-разному в разное время. Полномочие снимается только явным `agent.link_closed`; `deadline_seconds` стал необязательным и ничего не ограничивает — панель его больше не спрашивает. Границы агента считаются в попытках, `provider_units`, глубине и wall-time живого процесса, а не в календарном времени |
