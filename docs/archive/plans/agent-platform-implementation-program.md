...
Contract: ../../adr/README.md (ADR 0014-0019)
Superseded by: docs/plans/2026-09-18-consolidation-and-wave-4-plan.md
Source boundary: `605c0d7c9c5965c0a8303550056d07ce63afa697`
...
| Self-improvement dogfood — историческая граница | На `codex/service-self-improvement` от `e08488f392936c43d35f1013d6468fe45fa89b00` через UI созданы Codex frontend-роль, Claude reviewer и задача. Реальный Codex run оставил diff и завершился по таймауту 600 с; лид исправил refresh/race-сбои. Claude выполнил анализ, но получил `needs_operator`: финализатор перепутал task/review revisions. Дефект воспроизведён и исправлен в задаче `task.64Z9RGK58YFMR6GP9CBG3QPMFA`. Итоговый `make check`: 139 Work / 27 Gallery / 2107 Python passed, 13 прежних skips. Локальная установка обновлена; source/asset hashes совпадают, оба non-live preflight проходят. На той границе требовались свежие canaries и новые exact review; текущая квалификация описана в следующей строке; дополнительные запуски всех трёх провайдеров разрешены оператором 2026-09-08; прежние два canary относятся к старому fingerprint. Acceptance, merge, L1/R2 не закрыты. [Журнал, evidence и следующие P1](../../audits/2026-09-08-service-self-improvement-dogfood.md). |
| Project-native collaboration | Активная следующая волна: компактная навигация проектов, группы навыков и custom Blueprints, результаты конкретных задач/агентов, Human-in-the-Loop чат с изображениями и файлами. [ADR 0019](../../adr/0019-project-native-collaboration-and-outputs.md) и [граф реализации](project-native-collaboration-implementation-plan.md) фиксируют контракты, ownership и проверяемые gates. Изменения интегрированы в исходники; реальные Codex R3A и Grok R1 завершились `succeeded`, ответы Grok видны в чате задачи. Последующий установленный wheel совпадает с Python fingerprint рабочей копии; все шесть профилей Codex/Claude/Grok прошли реальные macOS canaries. Прежний лимит Claude больше не блокирует запуск. В установленном UI созданы отдельный проект, custom Blueprint, роль, задача и сообщение в чате. Тёплое чтение populated snapshot занимает около 0,32 с; холодный replay остаётся около 10 с. На границе 2026-09-09 (`a5ba8bf…` Python source) полный `make check` прошёл: 2586 Python-тестов, оба frontend-набора и Ruff. Проверены реальные media-сценарии с явными ограничениями каждого провайдера, preview после завершения агента и сохранённое изображение после завершения задачи. Эта версия установлена. Все девять задач затем переведены в независимый review по явному разрешению оператора; приёмка не выполнялась. Дополнительные review и полный прогон выявили гонку обновления UI и конкурентное создание delivery lock; обе правки отдельно проверены. Runtime `88531e7f…` установлен, все шесть Codex/Claude/Grok canaries повторно прошли. Итоговый `make check` прошёл: 2589 Python-тестов, 224 Work-теста, 27 Gallery-тестов и Ruff; 13 известных пропусков сохранены. Обновление evidence R5/R6 и Git delivery отслеживаются отдельно от приёмки задач. [Актуальные review bindings и delivery gates](../../audits/2026-09-09-project-native-review-delivery.md). |
| Provider transport / retention | Grok передаёт инструкции через фиксированный stdin endpoint без argv или файла с prompt; реальное выполнение подтверждено на текущей macOS-границе. Сохраняются отдельные Linux L1, retention/stale-coordination и release gates; успешный canary не заменяет их. См. [текущую проверку](../../audits/2026-09-08-project-native-collaboration-validation.md). |
| L1 / R2 | **L1 не доказан; release evidence R2 открыт.** Сначала безопасная проверка Linux receipt scope и green doctor; затем six-profile live qualification в рамках разрешения оператора от 2026-09-08 на точной source/provider/host границе, независимый review и решение по release. |
...
причина тех исторических Claude failures не установлена. Это не статус новых
шести macOS canaries выше. Broker остаётся experimental/manual.
Сам документ не выдаёт разрешение на provider runs; оператор отдельно разрешил дополнительные Codex/Claude/Grok запуски 2026-09-08. Конечные ограничения каждой попытки сохраняются.
