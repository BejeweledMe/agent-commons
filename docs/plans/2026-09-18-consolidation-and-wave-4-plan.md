...
## Решения владельца 21 сентября 2026 и следующий шаг

Основание: консилиум по фидбэку владельца
(`docs/handoffs/2026-09-21-council-product-direction.md`, запись в
`docs/evidence/2026-09-21/council/`) и догфудинг-отчёт
`docs/handoffs/2026-09-21-blizhe-dogfooding-feedback.md`.

| ID | Вопрос | Решение |
| --- | --- | --- |
| D-20 | Целевой пользователь | Через три месяца — open source; позже возможна коммерческая лицензия. Лицензия — Apache-2.0 плюс CLA/DCO с первого дня, чтобы сохранить право перелицензирования |
| D-21 | Local-first | Абсолютен. Трекер остаётся в приложении; Linear/Jira и экспорт в них не делаются; триггер возврата — второй реальный пользователь, живущий в трекере |
| D-22 | Визуальный фундамент | Шрифт Inter (OFL, self-hosted woff2, латиница и кириллица), иконки Lucide как SVG-спрайт, всё в бандле; макет «Предлагается» принят как референс (`docs/evidence/2026-09-21/council/mockups/`) |
| D-23 | Порядок | Сначала рабочий прогон end-to-end (UX-19, UX-21, UX-10, WP-20), потом визуальный фундамент; ведёт сессия ux-plan-lead |

Пакеты следующего шага (номера продолжают таблицу W4, не пересекаются с WP-30):

| Пакет | Исполнитель / review | Содержание | Критерии приёмки |
| --- | --- | --- | --- |
| WP-40 Прогон доходит до агента (выполнен 22 сентября тремя прогонами Terra; координатор снял регрессию 25 тестов запуска — обязательный контрольный файл только `.agent-commons/ONBOARDING.md`; тест на StopReason через пресет `qa-engineer`; манифест `docs/evidence/2026-09-22/wp-40-source-manifest.json`; **принят 22 сентября**, review.6CJBE14RVRK90GJ46JJM30JKEG approved с тремя неблокирующими замечаниями: снимок строится дважды за запуск и его диагностика не всплывает в run(), ветка parent-.git не форсируется фикстурой, пропуски из symlink-родителей и oversize не считаются) | Terra → Claude | UX-19: `ScopedRepoReader`/MCP snapshot обрабатывает не-файловые Git-entries (вложенные worktree, submodule-указатели) без следования symlink и без чтения чужих worktree; отказ обнаруживается в preflight до расхода provider attempt с типизированным кодом. UX-21: до расхода проверяется, что исполнитель получит рабочую копию, контрольные файлы и допустимые операции; причина остановки третьего слоя (`process_canonical_mismatch`, `probe`) не теряется — типизированная причина с кодом, человеческим текстом и одним действием попадает в canonical summary | Read-only воспроизведение UX-19 на семи вложенных worktree проходит; тест на каждый код отказа; `broker preflight` возвращает код, а не exit 1; попытка не расходуется при известном отказе |
| WP-41 Причина остановки в интерфейсе (выполнен 22 сентября: Opus — рендер StopReason/LaunchRefusal в карточке, истории и у формы запуска, UX-20; координатор через пресет `backend-builder` — аддитивное `stop_reason` в run-DTO; tsc чисто, 389 Node-тестов, 100 pytest; манифест `docs/evidence/2026-09-22/wp-41-source-manifest.json`; **принят 22 сентября**, review.77K4KKWX8B09S0MWFKMCD9J8CT approved; первый review-запуск сорвался по snapshot guard, потому что субагент ещё завершал проверки — урок: ждать уведомления субагента до запуска review) | Opus → Astra | UX-10: карточка прогона и история показывают безопасную причину и следующее действие из типизированной причины WP-40 (без stderr в первой строке; bounded stderr — в раскрытии); `needs_operator` несёт структурное «что именно нужно от вас» | Кадры EN/RU для трёх кодов отказа; тесты рендера; ни одной строки provider stderr в первом слое |
| WP-20 Ускорение записи | как в таблице W4; **сначала дизайн** (пресет `architect` → ADR-черновик, затем Terra) | Прогон Terra 22 сентября кода не менял: кэш проверенного префикса сам по себе не даёт безопасной suffix-only валидации — пересборка `ProjectSnapshot` и `ReceiptRecovery` требуют проверенного префикса; дизайн готов: ADR 0023 (черновик, пресет `architect`, 22 сентября) — измерено: warm `task_create` = 3,98 полных валидирующих прохода на запись, byte-identity проверка в 34–70× дешевле `read_path`; рекомендация A + B (in-process verified ledger view в write-lock + bulk-load receipts, ~0,33 с вместо ~10,9 с), durable-кэш C отложен как единственный вариант с поверхностью отравления; два прогона Terra | warm `task_create` ≤ 1 с на 3 206 событиях; `doctor` и golden replay неизменны |
| WP-42 Визуальный фундамент | Opus → Astra (после WP-40/41) | Inter и Lucide в бандле; шкала 12/13/15/18/22/28 при базовом 15px; `--border-subtle`; токены радиуса/тени/движения; слой `frontend/work/src/ui/` из ~10 примитивов на существующих токенах; прогрессивное раскрытие инспектора; интро-абзацы в пустые состояния; строка задачи 56px; одна навигация в сайдбаре; ULID моноширинным стилем | Ratchet и contrast-тесты зелёные; метрика до/после (строк текста над первой задачей, высота строки, число видимых рамок); слепой опрос 3–5 сторонних разработчиков |

Порядок с 21 сентября: WP-40 → WP-41 (параллельно WP-20 в отдельном worktree) →
WP-42 → далее по таблице W4 (WP-30 → WP-18 → WP-19 → WP-22 → WP-23; WP-17 уже
принят до WP-24). Хвосты волны 3 (двуязычные названия встроенного каталога,
WP-15.2, серверный гейт eligibility для preset) — в бэклог после WP-42.
