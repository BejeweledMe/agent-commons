...
Итог прохода (19 сентября, `doctor` ok): принято 23 задачи, закрыто
административно 20, отменено как superseded 21 (все pre-programme `active` и
`assigned`); в `review` остались четыре задачи — план консолидации (эта работа),
две с неприменённой старой приёмкой и одна свежая. Подтверждены 11 handoff,
адресованных всем; 94 handoff адресованы `role:operator` и подтверждаются только
из сессии с этой ролью (панель владельца) — они остаются открытыми. Предупреждения
`doctor` о stale review/verification не трогались: это факты о старых ревизиях, а
не ошибки; два decision с устаревшим evidence и orphan-манифест требуют
адресного repair владельца.
