...
| WP-11.2 canonical objective/application (реализация) | Terra | Claude, changes_requested (тесты отказов join и полей DTO) → approved (v2) | принят; 4 запуска Terra; координатор дописал тесты и нашёл дефект: `tracker_reads` проверял id применения как ULID вместо namespaced digest — `application_id` и унаследованная цель не доходили до DTO, исправлено по схеме. 2600 тестов, ruff чисто; `wp-11.2-source-manifest-v2.json` |

Замечания reviewer-а по WP-11.2 (не блокирующие, к отработке): (1) тест
`test_a_partial_earlier_write_is_completed_without_a_second_application`
доказывает повтор, но не моделирует обрыв между последней задачей и записью —
переименовать или дописать сценарий; (2) «старый» ledger в golden replay пишется
текущим writer-ом (payload v2 с `objective_id: null`), настоящий v1-payload без
ключа не проверяется; (3) манифест перечисляет сам себя, поэтому одна из 31
записей принципиально непроверяема — исключать собственный путь из списка файлов.
...
Решения владельца 11 сентября 2026 (по вопросам координатора с trade-offs):
ADR 0021 принят, WP-11.2 реализуется сейчас, до волны 3; волна 2 — коммит, PR #13,
CI, merge, рестарт 8787; продуктовые p50/p95 не фиксируются, подсказки 2 с/8 с
остаются презентационными (`ux/latency-thresholds`,
decision.2C8F1CEMD4A8DF88NK3XF7FMJ4, заменил решение от 9 сентября).

## N. Волна 3 (начата 11 сентября 2026)

Порядок: WP-13.1 (Grok, механическая консолидация строк и инвентаризация токенов)
→ WP-09 → WP-10 (Opus) → WP-12.1 (Terra) → WP-12.2 (Opus) → WP-17.1 (Terra) →
WP-17.2 (Opus) → WP-15.3 (Grok) → WP-13.2 (Opus); интеграция asset — координатор
в конце волны. Ветка `wave-3-ux-closure` (уже содержит принятый WP-11.2).

| Пакет | Исполнитель | Review | Итог |
|---|---|---|---|
| WP-13.1 инвентаризация строк и цветов | Grok | Astra, approved 18 сентября (первый review умер вместе с машиной 11 сентября; делегирование 17 сентября осталось незапущенным из-за истёкшей сессии координатора и восстановлено через `delegation recover`) | **принят** 18 сентября. 202 строки перенесены в один реестр с namespace-префиксами, модули остались тонкими shim-ами, snapshot-тест побайтно; паритет EN/RU на 933 ключах; инвентарь цветов/отступов/типографики + ratchet-линт; 304 теста, tsc чисто; `wp-13.1-source-manifest.json` |
| WP-09 «Сейчас / Карта / Все задачи» внутри вкладки Задачи | Opus | Astra, approved 18 сентября | **принят** 18 сентября. Новый `TaskViews.tsx` — единственное место рендера списка, графа и трёх колонок «Сейчас»; чистая `bucketTasksForNow` (needs_you / in_progress / next, принятые и отменённые в невидимой группе `settled` со ссылкой на «Все задачи»); ёмкость графа решается до монтирования, старый двойной рендер списка-фолбэка удалён; параметр `tasks=now|map|all` только презентационный; 13 новых ключей EN/RU; координатор поправил две тестовые фикстуры (сортировка ожидания, ULID-подобные id для 300 задач); tsc чисто, 312 тестов; `docs/evidence/2026-09-18/wp-09-source-manifest.json` |
| WP-10 DecisionCard и иерархия инспектора | Opus | Astra, approved в раунде 3 (18–19 сентября) | **принят** 19 сентября. Первый запуск 18 сентября погиб вместе с сессией координатора до первой правки (delegation.5M46CWVRBG57NCKKVJ7V22B4X0 → needs_operator через `broker reconcile`); перезапуск delegation.17QKTQZKB754XVNB16KEFDXJZR завершился успешно; координатор сделал `findings` в карточке тройственными (`null` = сервер не прикрепляет находки к записи задачи, честная подпись вместо «0»); tsc чисто, 324 теста; отправлен на review (review.7821XY6XEZPVMY9AEBRHA2F3TE); первый review-запуск Астры упал с `provider_reported_error`; второй (delegation.7BP5QNKFR5M2M9M0A4H039S9V2) отработал, но вердикт не записался: в 20:49 UTC параллельная hygiene-сессия (decision.1H8KMRVD0A45BAXS1A1D9DYF6J) сделала `task.reopened` для task.5Y22M96JPZHS8TM9J9XP9WF6TV с устаревшей expected_revision, `doctor` фиксирует конфликт переходов, и все записи в леджер отвергаются; после инвалидации evt.01M2V4GDKY3KWC0MHM6R9CPFGT (леджер здоров) третий review вынес `changes_requested`: accept/return слали свежую ревизию из `/graph`, а не отрисованную. Координатор починил: инспектор отдаёт панели действий отрисованную ревизию (render-prop `RenderedTask`), intent несёт `expectedRevision`, `acceptTask`/`reopenTask` требуют её как CAS без чтения `/graph`, неотрисованная/битая ревизия отклоняется на клиенте; регрессионные тесты; tsc чисто, 327 тестов; манифест v2. Раунд 2: починка признана верной, но запрошен регрессионный тест на повтор неудавшегося intent; добавлен (повтор с тем же ключом после перерисовки шлёт байт-в-байт то же тело с исходной ревизией, новую берёт только явный «начать заново»); 328 тестов; манифест v3 `wp-10-source-manifest-v3.json`; раунд 3 approved |
| WP-10.1 находки в записи задачи (бэкенд-половина WP-10) | Terra | claude-independent-reviewer, approved 19 сентября (два неблокирующих замечания: срез после материализации всех совпадений и список вне бюджета bounded_copy; линейный скан коллекции находок на каждое чтение) | **принят** 19 сентября: `record.findings` (id, title, severity, state) для находок, чьи evidence_refs привязаны к задаче, всегда присутствует, cap 50 + `findings_truncated`; чистый helper `findings_for_task`; 4 теста против read-only панели; фраза в FRONTEND_CONTRACT; координатор отформатировал тестовый файл; 31 тест, ruff чисто; `wp-10.1-source-manifest.json` |
| WP-12.1 eligibility DTO и отказ при найме | Terra | claude-independent-reviewer, approved 19 сентября (неблокирующее: review-роль определяется по суффиксу `-reviewer`; найм по preset без специализации не проверяется; `writer()` создаётся до проверки; чип `review_only` деградирует молча; точка входа `create_agent`) | **принят** 19 сентября. Запуск 1 (19 сентября) вышел с запросом ввода после частичной реализации: `services/worker_eligibility.py` (правила: нерезолвимая специализация → unknown; несвежие qualification/auth → unknown; reviewer-профиль на не-review роль → ineligible; не launchable → ineligible с отказом провайдера; иначе eligible), `GET /api/workers/eligibility`, отказ 409 `worker_ineligible` до записи в `create_role`, абзац в FRONTEND_CONTRACT, один тест; координатор проверил (68 тестов, ruff чисто). Checkpoint-запуск 2 только на матрицу тестов: первое делегирование отклонено по квалификации (receipt `codex-builder` устарел после правок src в run 1) → переустановка, canary, delegation.3X6X39JNAVB16MTGMBZHVV9JM7 добавил `tests/ui/test_worker_eligibility.py` (9 тестов, (a)–(i)); 69 тестов, ruff чисто; `wp-12.1-source-manifest.json` |
| WP-12.2 role-first найм (специализация → eligible-воркеры, чипы, Advanced) | Opus | Astra, approved в раунде 2 (19 сентября) | **принят** 19 сентября. Выполнен: новый `WorkerPicker.tsx` (idle/loading/ready/unavailable; выбираемы только eligible из текущего ready-ответа, ineligible/unknown видимы и отключены с отказом и remediation-глоссарием, сбой чтения = все unknown), строгий парсер `parseWorkerEligibility`, чипы возможностей как текст, provider/model в закрытом Advanced с дефолтами воркера, 409 `worker_ineligible` рядом с шагом воркера с сохранением черновика и перечитыванием, тело `createRole` без изменений, +28 ключей EN/RU; tsc чисто, 339 тестов; манифест v1. Раунд 1 (Astra): legacy preset обходил гейт eligibility (исключение `role.fromPresetId`). Координатор починил: общий чистый гейт `hireGate.ts` (`hireWorkerAllowed`) — единственный гейт формы, без исключения для preset; выбор preset-а подставляет его `specializationRef`, так что eligibility читается по нему, preset без специализации остаётся заблокирован; регрессионный тест по всем не-ready состояниям и не-eligible вердиктам; 340 тестов; манифест v2 `wp-12.2-source-manifest-v2.json`; раунд 2 approved |
| WP-17.1 локальное opt-in content-free хранилище инструментирования | Terra | claude-independent-reviewer, approved 19 сентября (неблокирующее: `settings()` обновляет день retention, так что переключение откладывает сброс; `updated_at_day` фиксирует день последнего использования панели) | **принят** 19 сентября. Выполнен: `ui/instrumentation.py` (один JSON-файл 0600 под resolved state root, схема `agent_commons.instrumentation.v1`, enabled=false по умолчанию, закрытые enum-ы 7×4×4, cap 64 KiB и 64 событий на вызов, атомарная замена, retention только по дню, ревизия = sha256 канонического документа, без сетевых импортов); приватные revision-bound маршруты GET/settings/events/clear (204 при выключенном, 422 вне enum, 409 stale); read-only панель без записей; раздел приватности в FRONTEND_CONTRACT, строка в THREAT_MODEL; 5 тестов; координатор: 55 тестов, ruff чисто; `wp-17.1-source-manifest.json` |
| WP-17.2 переключатель и раскрытие инструментирования в Settings | Opus | Astra, approved 19 сентября | **принят** 19 сентября. Выполнен: `instrumentation.ts` (чистый mapper в закрытые enum-ы, батчер ≤64 без replay, инертен при выключении), `InstrumentationSettings.tsx` (переключатель, раскрытие простыми словами, итоги kind×outcome, «Очистить» с подтверждением, read-only рендер, никакого экспорта), семь точек эмиссии с литеральными kind и outcome по статусу, +35 ключей EN/RU; координатор: `requestOutcome` перенесён ниже `isObject` (срез старого теста), `ui_version` закреплён «1», одна EN-строка без апострофа; tsc чисто, 358 тестов; `wp-17.2-source-manifest.json` |
| WP-15.3 `wall_time_seconds` в receipt квалификации и availability DTO | Grok | Astra, approved 19 сентября | **принят** 19 сентября. Выполнен (Grok снова вышел без терминального вызова — finding.7WEDARP2X3JXJ2E57JSQ499BPC; отчёт сгенерировал координатор): `ProviderQualification.wall_time_seconds` (30…1800, опционально; reader принимает receipt с полем и без; схема и формула fingerprint без изменений), canary пишет использованное время в receipt, блок qualification в availability DTO несёт `wall_time_seconds` int\|null, тесты round-trip/legacy/out-of-range/оба DTO, фраза в FRONTEND_CONTRACT; координатор исправил одну синтаксическую опечатку и отформатировал два файла; 230 тестов, ruff чисто; `wp-15.3-source-manifest.json` |
| WP-13.2 тёмная семантическая система токенов (только CSS) | Opus | Astra, approved в раунде 2 (20 сентября) | **принят** 20 сентября. Выполнен 19 сентября: 47 токенов в `:root` (surface/text/border/status/accent + шкалы отступов и типографики), legacy-имена стали алиасами, raw hex вне `:root` = 0 во всех пяти файлах (было 226/2/1/0/0), ratchet закреплён на 0, новый `contrast.test.mjs` (WCAG AA от объявленных значений); видимые эффекты: рамка-разделитель чуть светлее (#455873 → #52698c ради 3:1), hover primary-кнопки темнее; ни одного `.ts/.tsx` не тронуто; tsc чисто, 363 теста; `wp-13.2-source-manifest.json`; asset пересобран координатором; визуальная проверка без регрессий. Раунд 1 (Astra): в `taskGraph.css` остались два сырых зелёных (файл был вне ratchet) и критерий назвал брейкпоинты 760/700 вместо реальных 780/700. Координатор токенизировал `taskGraph.css` целиком (accepted-узлы на `status-accepted-*`, blocked на attention), добавил файл в ratchet и инвентарь, обновил тесты графа; 364 теста; asset пересобран; манифест v2 `wp-13.2-source-manifest-v2.json`; раунд 2 approved |

### Интеграция волны 3 (19 сентября)

- Браузерная проверка демо-workspace выявила дефект интеграции: бэкенд WP-15.3
  добавил `qualification.wall_time_seconds` в availability DTO, а строгий
  клиентский парсер (`hasExactKeys`) отвергал новый ключ — вкладка Tasks
  показывала `request_unavailable`, Settings — «availability could not be read».
  Координатор аддитивно принял поле на клиенте (int 30…1800 или null, отсутствие
  = «не предоставлено», `qualification.wallTimeSeconds`), добавил тест обеих
  форм DTO; 364 теста. Урок записан как finding: «аддитивное» поле сервера
  аддитивно только после того, как клиентский exact-key парсер его допускает.
- Asset пересобран координатором на Node 24; тест свежести бандла проходит.
- Визуальный проход по демо-workspace (127.0.0.1:8792): вкладка Tasks с видами
  «Сейчас / Карта / Все задачи», DecisionCard (три вопроса, одна primary-кнопка,
  «Findings (0)» от сервера, закрытые Details), форма найма с WorkerPicker
  (воркеры unknown с причиной и remediation, чипы, Advanced), раздел
  «Local usage counters» в Settings в read-only и настроенном состояниях —
  регрессий не найдено. Попутно обновлена подсказка `role_help` (EN/RU) под
  role-first найм. Наблюдение вне волны: названия встроенных специализаций
  показываются по-русски при EN-локали интерфейса — на триаж.
- Интеграционный манифест `wave-3-integration-manifest.json`
  (artifact.1RPMQ93TX0QE5GWJK18NGJM7NG). После приёма WP-13.2 — по команде
  владельца: полный `make check`, коммит, PR, мердж, рестарт панели 8787.
