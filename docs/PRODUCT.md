...
| Agent / Агент | A named instance of a specialization inside a project: name, provider, profile, model, pinned skill versions. Target first-level word; the shipped UI still says "role" in the hire form, the model hint and the board introduction (`Role name`, `role_model_help`, `board_intro`) until WP-12.2/WP-23 rename those strings | ledger `agent`, historically and in several current strings "role" |
