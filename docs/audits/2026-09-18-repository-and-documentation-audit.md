...
Дата: 18 сентября 2026, числа карты кода обновлены 20 сентября. Автор:
координатор (Claude Fable), сессии `session.8f29684ae9824337a0f29a9ea73fdae9`
и `session.e4e94e8fb5594377badf7c7b0456588b`. Граница проверяемой ревизии:
`main` на `c68cee1` (волна 3 влита 20 сентября) плюс ветка `docs-consolidation`
на `78b855d`. Первичный снимок 18 сентября снимался на `43b7d8f` с
незакоммиченной волной 3; разделы 1.4 и 2 описывают состояние программы на ту
дату и помечены так явно. Метод: четыре независимых read-only прохода (карта
кода, гигиена файлов, противоречия документов, покрытие UX-фидбэка по леджеру)
и ручная проверка ключевых утверждений по исходникам.
...
| --- | --- | --- | --- |
| `src/agent_commons/` | 682 файла | 16M | Python-пакет: CLI, домен, сервисы, storage, runtime, UI-сервер, MCP |
| `frontend/` | 131 файл (work + gallery) | 147M на диске (node_modules) | React 19 + Vite 8 + TS: приложение Work (Доска · Задачи · Агенты · Библиотека · Настройки) и галерея дизайн-пакетов (`/gallery`) |
| `tests/` | 220 файлов | 16M | 206 pytest-модулей в 19 каталогах |
| `docs/` | 244 файла | 4.5M | 22 ADR, 11 документов верхнего уровня, 2 активных плана, 33 файла аудитов, 124 evidence-файла, 35 файлов архива, руководства (после консолидации; до неё — 13 планов верхнего уровня и два review в корне репозитория) |
| `.agent-commons/` | 3 файла | 17M локально | Леджер (~4 000 событий, ~400 манифестов) — игнорируется Git, кроме `ONBOARDING.md`, `workspace.yaml`, `.gitignore` |
...
| `docs/archive/reviews/claude_architecture_improvement_review.md`, `…/codex_architecture_improvement_review.md` | 2 | 140K + 48K | Ревью августа; до консолидации лежали в корне; хэши в `docs/reviews/README.md` не изменились |
| `CHANGELOG.md` | 1 | 31K → дописан | До консолидации последняя запись — август 2026 (без волн 1–3, доски, проектов, результатов); в ветке `docs-consolidation` доведён до `43b7d8f` |
...
| --- | --- | --- | --- |
| `runtime/` | 27 | 14 121 | Провайдеры, попытки, вложения, live previews, qualification |
| `services/` | 35 | 13 485 | Менеджер леджера, задачи, делегирования, review, outputs, conversations, локальные счётчики |
| `ui/` | 33 | 12 975 | FastAPI-сервер, маршруты Work/Gallery/Board, DTO трекера, eligibility, сессия панели |
...
1 972, `ui/server.py` 1 619, `mcp/server.py` 1 583, `services/manager.py` 1 404,
...
| --- | --- | --- | --- |
| Work (продукт) | `frontend/work/src` (32 компонента, 41 модуль, 28 node-тестов) | `src/agent_commons/ui/static/work/assets/work-<hash>.js` — один хэш, коммитится | `agent-commons ui` печатает `/work#c=…` (`cli/__init__.py:364`) |
...
### 1.4. Состояние программы закрытия UX (по леджеру, снимок 18 сентября)

Раздел описывает состояние на `43b7d8f` с незакоммиченной волной 3; 20 сентября
волна 3 влита (`c68cee1`, PR #15), а леджер прошёл гигиену C7 (итог — в плане).
...
## 2. Покрытие UX-фидбэка от 9 сентября (снимок 18 сентября)

Статусы ниже относятся к `43b7d8f` с незакоммиченной волной 3. После merge
волны 3 (20 сентября) пакеты WP-09, WP-10, WP-12.1/12.2, WP-13.2, WP-15.3 и
WP-17.1/17.2 приняты; текущий статус каждого UX-пункта ведёт активный план.
...
| Роль / Role | Целевое правило: слово леджера для записи агента (`agent`); в первом уровне UI заменяется на «агент». Сегодня UI ещё говорит «роль» в форме найма, подсказке о модели и вступлении доски (`i18n.json`: `Role name`, `role_model_help`, `board_intro`); переименование этих строк — отдельный пакет WP-30 плана консолидации, до него оба слова сосуществуют | Заменить в строках UI пакетом WP-30; поля леджера не переименовывать |
