...
1. **Сначала профилировать и убрать этот фиксированный пол.** Первый кандидат —
   ...
   перед реплеем. Профиль обязателен — и он определяет не только слагаемое, но и
   путь: замеренный здесь `CommonsManager.snapshot()` читает канонические файлы
   событий мимо SQLite (E2 лежит на UI-пути `_read_snapshot`), а бенчмарк A0
   реплеит список в памяти вообще без I/O. Арифметика «~2 с фиксированного
   пола» вычитает замеры с разной стоимостью путей, поэтому ни кандидата, ни
   само слагаемое нельзя принимать на веру до профиля.
