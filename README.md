...
it offers to create the project, find or configure a provider-backed runtime,
hire a role with a chosen model, and put it to work — the
...
The panel can create the project and discover `claude` or `codex`, but it cannot
run work without one of those provider CLIs installed and available through the
operator's subscription. If neither is found, the first-run screen says that
run functionality is unavailable, links here, and offers **Look again** after
installation; it does not simulate a completed run.

When discovery finds a provider, the panel writes its generated configuration
once to the frozen operator path. A later click can add profiles for a newly
discovered provider only when the existing file byte-for-byte proves it was
generated by the panel. Hand-edited files are never merged or overwritten: the
panel gives a typed refusal and leaves the operator to edit them manually.

These limits follow
[`decision.2ZTHNGPQVMHZ5RF614HQPWCKYV`](docs/THREAT_MODEL.md#first-run-setup-as-a-write-surface)
and
[`decision.1A08MD6B8TXRWVNX00DJZD98DY`](docs/THREAT_MODEL.md#first-run-setup-as-a-write-surface):
demo is not a product onboarding path, and generated configuration ownership is
proved before additive regeneration.
