...
export type BlueprintDraftError = "identity" | "text" | "slots" | "roles" | "tasks" | "dependencies" | "cycle";
export function blueprintDraftErrors(draft: BlueprintDraft, catalog: ServiceLibrary, plans: readonly WorkBlueprint[] = []): BlueprintDraftError[] {
  ...
  for (const locale of ["en", "ru"] as const) if (!validText(draft.name[locale], 800) || !validText(draft.description[locale], 16000)) errors.add("text");
  ...
  for (const task of draft.tasks) {
    ...
    for (const locale of ["en", "ru"] as const) if (!validText(task.title[locale], 800) || !validText(task.description[locale], 16000) || !task.acceptance_criteria[locale].length || task.acceptance_criteria[locale].length > 32 || task.acceptance_criteria[locale].some((criterion) => !validText(criterion, 16000))) errors.add("text");
