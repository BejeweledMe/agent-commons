...
export function ServiceLibrarySection({ api, state, onRefresh, onChooseRole, text, writesEnabled, kind }: {
  api: LibraryApi; state: LibraryState; onRefresh: () => void; onChooseRole: (role: LibraryRole) => void;
  text: (key: MessageKey) => string; writesEnabled: boolean; kind?: "role" | "skill";
}): ReactElement {
  ...
  return <section ref={container} className="service-library" aria-label={text(tab === "role" ? "library_roles" : "library_skills")}>
    <div className="composer-heading"><div><h2>{text(tab === "role" ? "library_roles" : "library_skills")}</h2><p className="lead-copy">{text(tab === "role" ? "library_catalog_intro" : "library_skills_intro")}</p></div><button type="button" className="button button-secondary" onClick={onRefresh}>{text("refresh_status")}</button></div>
